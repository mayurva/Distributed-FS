...
//		printf("Recvd %d\n%s\n",n,tcp_buf);
		...
		printf("open ends\n");
                ...
                {
		  int res,fd,flags,tot=0,cacheflag=0,offset;
		  printf("Received message is %s\n",a);fflush(stdout);
		  a = strtok(NULL,"\n");
		  path = (char*)malloc(strlen(rootpath)+strlen(a)+5);
		  strcpy(path,rootpath);
		  ...
		  if (fd != -1)
		      ...
		      while(strcmp(tcp_buf,"ok")==0)
			{
			  ...
			  tcp_buf[res]='\0';
			  if(strcmp(tcp_buf,"cache")==0){cacheflag=1;}
			  if(strcmp(tcp_buf,"done")!=0 && strcmp(tcp_buf,"next")!=0 && strcmp(tcp_buf,"cache")!=0)
			    {
			      if(strcmp(tcp_buf,"ok")!=0)
				{
				  printf("In if");
				  offset=atoi(tcp_buf);
				  ...
				  printf("here : : %s : \n\n%d:",tcp_buf,strlen(tcp_buf));fflush(stdout);
				  if(strcmp(tcp_buf,"")==0)
				    {
				      printf("\n\nfile finished\n\n");fflush(stdout);
				      send(clientList[n].conn_socket,"file_is_finished",strlen("file_is_finished"),0);
				      tot=tot+rflag;
				    }
				  else
				    {
				  ...
				  tcp_buf[res]='\0';
				  printf("\n%s",tcp_buf);fflush(stdout);
				  //offset=offset+BLOCKSIZE;
				}
			    }
			  send(clientList[n].conn_socket,"hi",strlen("hi"),0);
			}
		  ...
		  close(fd);
		  printf("cacheflag %d",cacheflag);fflush(stdout);
		  memset(tcp_buf,0,MAXLEN);
		  /*	  if(cacheflag==0){
		  recv(clientList[n].conn_socket,tcp_buf,MAXLEN,0);
		  memset(tcp_buf,0,MAXLEN);
		  ...
		  if (fd != -1)
			...
			exit(1);
		      ...
		      send(clientList[n].conn_socket,"success",strlen("success"),0);
		      memset(tcp_buf,0,MAXLEN);
		      recv(clientList[n].conn_socket,recvbuf,(int)size,0);
		     /* if(res<0){
			printf("\nError receiving flags");
			exit(1);
		      }
